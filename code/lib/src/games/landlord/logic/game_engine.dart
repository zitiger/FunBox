...
class GameSession {
  ...
  Map<String, Object?> toJson() {
    return {
      'hands': hands
          .map((h) => h.map((c) => {'suit': c.suit.index, 'rank': c.rank}).toList())
          .toList(),
  ...
  factory GameSession.fromJson(Map<String, Object?> json) {
    final hands = (json['hands'] as List)
        .map((h) => (h as List)
            ...
            .toList())
        .toList();
    final holeCards = (json['holeCards'] as List)
        .map((c) {
          final m = c as Map;
          return PlayingCard(CardSuit.values[m['suit'] as int], m['rank'] as int);
        })
        .toList();
    final lastPlay = (json['lastPlay'] as List)
        .map((c) {
          final m = c as Map;
          return PlayingCard(CardSuit.values[m['suit'] as int], m['rank'] as int);
        })
...
class GameEngine {
  ...
  static String? validatePlay(List<PlayingCard> cards, GameSession session) {
    ...
    if (session.lastPlay.isEmpty && session.lastPlayPlayer == session.currentTurn) {
      return null;
    }
