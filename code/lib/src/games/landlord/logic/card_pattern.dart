...
class PatternRecognizer {
  static CardPattern? recognize(List<PlayingCard> cards) {
    ...
    if (sorted.length == 2) {
      if (_isRocket(sorted)) {
        return CardPattern(PatternType.rocket, PlayingCard.rankBigJoker, sorted);
      }
      if (_isPair(sorted)) {
  ...
  static CardPattern? _tryStraight(List<PlayingCard> sorted, Map<int, int> ranks) {
    if (sorted.length < 5) return null;
    for (final c in sorted) {
      if (c.rank >= PlayingCard.rank2 || c.isJoker) return null;
    }
    for (int r = sorted.first.rank; r < sorted.first.rank + sorted.length; r++) {
      if ((ranks[r] ?? 0) != 1) return null;
    }
    return CardPattern(PatternType.straight, sorted.last.rank, sorted);
  }

  static CardPattern? _tryConsecutivePairs(List<PlayingCard> sorted, Map<int, int> ranks) {
  ...
  static CardPattern? _tryTripleWithOne(List<PlayingCard> sorted, Map<int, int> ranks) {
  ...
  static CardPattern? _tryTripleWithPair(List<PlayingCard> sorted, Map<int, int> ranks) {
    if (sorted.length != 5) return null;
    int? tripleRank;
    int? pairRank;
    for (final r in ranks.keys) {
    ...
    for (final r in ranks.keys) {
      if (ranks[r]! >= 3) {
        ...
        for (final remR in PatternRecognizer._rankCounts(remaining).keys) {
          if (PatternRecognizer._rankCounts(remaining)[remR]! >= 2) pairsInRemaining.add(remR);
        }
        if (pairsInRemaining.isNotEmpty) {
          final pr = pairsInRemaining.first;
          final pairCards = remaining.where((c) => c.rank == pr).take(2).toList();
